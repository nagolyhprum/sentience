...
func _process(delta: float) -> void:
	var offset : float = random_offset + Time.get_ticks_msec() / 250.0 / days_per_orbit
	var scale_offset = cos(offset) * distance / 20.0
	...
	position.x = -half_size.x + sin(offset) * distance * 1000.0
